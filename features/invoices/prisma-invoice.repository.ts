...
import { type InvoiceDto, type UpsertInvoiceData } from "./invoice.schema";
...
function mapToDto(row: any): InvoiceDto {
  return {
    id: row.id,
    number: row.number,
    status: row.status,
...
export class PrismaInvoiceRepo extends BaseRepository {
  private get companyWhere() {
    return { companyId: this.user.companyId };
  }

  async getAll(params?: GetQueryParams): Promise<{ items: InvoiceDto[]; total: number }> {
  ...
  @Transaction
  async upsert(data: UpsertInvoiceData): Promise<InvoiceDto> {
    const { companyId } = this.user;
    ...
    const maxRecord = await this.prisma.invoice.findFirst({
      where: { companyId },
      orderBy: { number: "desc" },
      select: { number: true },
    });
  ...
  @Transaction
  async delete(id: string): Promise<InvoiceDto> {
    const { companyId } = this.user;
    const row = await this.prisma.invoice.findFirstOrThrow({
      where: { id, companyId },
      select: invoiceSelect,
    });
